...
let comMove;
  ...
  if(count>=9)
      {
        if(titleRef.current!==null)
          (titleRef.current as HTMLElement).innerHTML=`Tie`
      }
  ...
  const onePlayerMode=(e:React.MouseEvent<HTMLDivElement, MouseEvent>,num:number)=>{
    ...
    if((e.target as HTMLElement).innerHTML===` `)
      {
      (e.target as HTMLElement).innerHTML = `<img src='${cross_icon}'>`;
      data[num]="x";
      setCount(++count);
   ...
   if(checkWin()||count>=9)
    return 0;
        ...
        if(comMove!==undefined)
        if (box_array[comMove].current !== null) {
          (box_array[comMove].current! as HTMLElement).innerHTML = `<img src='${circle_icon}'>`;
      }
        data[comMove]="o";
        setCount(++count);
}
  ...
  const findBestMove=(data:string[])=>{
    const findMove=(data:string[])=>{
      let i=0;
      while(data[i]!==""&&i<9)
        i++;
      if(i<9)
...
return 0;
    ...
    if(win!==-1&& win !==undefined)
      {
        return win;
      }
      else if(block!==-1&& block!==undefined){
        return block
            }
    else if(mid!==-1&& mid !==undefined)
      {
        return mid;
      }
  else if(opt!==-1&& opt !==undefined)
  return opt;
return findMove(data);
  }
  const possibleWin=(data:string[])=>{
  ...
  const possibleBlock=(data:string[])=>{
  ...
  const neighbors=(data:string[],i:number)=>{
  switch(i){
    case 0:
      if(data[1]==="")
        return 1;
  ...
  const twoPlayerMode=(e:React.MouseEvent<HTMLDivElement, MouseEvent>,num:number)=>{
    if(lock){
      return 0;
    }
      ...
      if((e.target as HTMLElement).innerHTML===` `){
      (e.target as HTMLElement).innerHTML = `<img src='${circle_icon}'>`;
      data[num]="o";
      setCount(++count);
      }
    }
  ...
  const won = (winner:string)=>{
    ...
    if(winner==="x")
      {
        if(titleRef.current!==null)
          (titleRef.current as HTMLElement).innerHTML=`Congratulations  : <img src='${cross_icon}'>  Won`
      }
      
    else if(titleRef.current!==null){
      (titleRef.current as HTMLElement).innerHTML=`Congratulations :  <img src='${circle_icon}'>  Won`
    }
  ...
  const reset = ()=>{
    ...
    data=["","","","","","","","",""];
    if(titleRef.current!==null)
      (titleRef.current as HTMLElement).innerHTML='Tic Tac Toe Game In<span>React</span>';
     // eslint-disable-next-line
     box_array.map((e)=>{
      if(e.current!==null)
      (e.current as HTMLElement).innerHTML=` `;
     })
  }
  const p1: React.MouseEventHandler<HTMLButtonElement> = (event) => {
...
setActiveButton('p1');
  ...
  const p2: React.MouseEventHandler<HTMLButtonElement> = (event) => {
